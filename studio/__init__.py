...
* ``studio.schema`` and ``studio.science`` must import NOTHING from the API, the database, the web
  layer, or ``coupled``. Importing the model (``coupled.driver`` / ``coupled.tomas_bridge``) pulls
  in JAX and TOMAS, which an API validating a form on every keystroke cannot afford. Both packages
  must be usable from a bare Python session.
