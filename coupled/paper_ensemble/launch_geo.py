...
"""Launch the 540-case geoengineered-background ensemble (run_geo_ensemble.py) across N
thread-pinned worker processes. Same scheme as launch_parallel.py: contiguous slices, one
summary_<lo>_<hi>.csv per worker, merged into summary.csv at the end.

Run (from SANDBOX/):  python -m coupled.paper_ensemble.launch_geo [n_proc]   (default 10)
"""
import csv
import glob
import math
...
_OUT = os.path.join(_HERE, "runs_geo")
_TOTAL = 540
...
def main(n_proc=10):
    ...
    for k in range(n_proc):
        ...
        p = subprocess.Popen(
            [sys.executable, "-m", "coupled.paper_ensemble.run_geo_ensemble",
             "run", str(lo), str(hi)],
