...
def test_to_model_config_maps_shared_fields():
    ...
    assert mc.M is not None   # ModelConfig computes air number density


def test_initial_state_pptv_to_number_density():
    from config import IDX, N_SPECIES, air_number_density
    sc = CoupledScenario(P=68.0, T=210.0, concentrations={"SO2": 1.0e6, "OH": 0.5})
    x = initial_state(sc)
...
def test_initial_state_rejects_unknown_species():
    ...
    sc = CoupledScenario(concentrations={"SO2": 1.0, "NOPE": 5.0})
