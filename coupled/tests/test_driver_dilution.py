...
def test_dilution_off_changes_nothing_vs_baseline():
    # dilution OFF must be identical to a run without the switch (no silent effect)
    t0, x0 = run_coupled(_sc(switches=Switches(sulfur=True, dilution=False)))
    t1, x1 = run_coupled(_sc(switches=Switches(sulfur=True)))   # dilution default False
...
def test_strong_dilution_suppresses_accumulated_product():
    ...
    h2so4 = IDX["H2SO4"]
    _t, x_off = run_coupled(_sc(switches=Switches(sulfur=True, dilution=False)))
    _t2, x_on = run_coupled(_sc(switches=Switches(sulfur=True, dilution=True), dilution_rate=1.0e-2))
...
def test_dilution_pulls_toward_background():
    ...
    _t, x_off = run_coupled(_sc(switches=Switches(sulfur=True, dilution=False)))
    _t2, x_on = run_coupled(_sc(switches=Switches(sulfur=True, dilution=True), dilution_rate=1.0e-4))
    from config import IDX
    so2 = IDX["SO2"]
